...
static void counter_updated(const char *);
...
static std::string sensor_update_interval_s;
static std::string sensor_type;
...
static std::string get_env(const char *key, const char *defaultval) {
...
{
    // sensor to configure
    sensor_type = get_env("SENSOR", /*default*/ "vibration");
    ...
    // update interval
    sensor_update_interval_s = get_env("INTERVAL", /*default 5secs*/ "5");
    ...
    // init commander
    commander = new Commander(pdmc_client, blinky);
    commander->listen();
}
...
#ifndef PDMC_EXAMPLE_MINIMAL
    blinky.init(pdmc_client,  commander, sensed_res, std::stol(sensor_update_interval_s), sensor_type);
...
bool create_pdmc_resources()
{
    if (sensor_type == "vibration" || sensor_type == "temperature" || sensor_type == "counter") {
        if (sensor_type == "vibration") {
            sensed_res = M2MInterfaceFactory::create_resource(object_list, 3313, 0, 5700, M2MResourceInstance::INTEGER, M2MBase::GET_PUT_ALLOWED);
        } else if (sensor_type == "temperature") {
            sensed_res = M2MInterfaceFactory::create_resource(object_list, 3303, 0, 5700, M2MResourceInstance::INTEGER, M2MBase::GET_PUT_ALLOWED);
        } else if (sensor_type == "counter") {
            sensed_res = M2MInterfaceFactory::create_resource(object_list, 3200, 0, 5501, M2MResourceInstance::INTEGER, M2MBase::GET_PUT_ALLOWED);
        }
        ...
        sensed_res->set_observable(true);
        sensed_res->set_message_delivery_status_cb((void(*)(const M2MBase &, const M2MBase::MessageDeliveryStatus, const M2MBase::MessageType, void *))delivery_status_cb, NULL);
        sensed_res->set_value_updated_function(counter_updated);
    }
